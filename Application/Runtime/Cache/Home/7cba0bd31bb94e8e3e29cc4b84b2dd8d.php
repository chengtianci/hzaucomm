<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
 	<meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.0; user-scalable=0;" />
 	<title>实验教学中心互动平台</title>
	<link rel="stylesheet" href="/hzaucomm/Public/face/css/public.css"/>
	<link rel="stylesheet" href="/hzaucomm/Public/face/css/comp.css"/>
	<link rel="stylesheet" href="/hzaucomm/Public/face/public/icon.png">
</head>
<body>
	<div id="Cheader">
		<div class="container">
  <div id="header_left">
    <a href="/hzaucomm/index.php/home/Index/index.html"><h1>华中农业大学作物学实验教学中心<h2>互动平台</h2></h1></a>
    <p>     Teaching Center of Crop Science Experiment | Plat of Communication</p>
  </div>
  <div id="header_right">
      <div id="login">
        <p><a href="#">学生登录</a> | <a href="#">教师登录</a></p><br/>
      </div>
      <form action="#" method="post" id="sitesearch">
        <input type="text" value="" placeholder="全文搜索" class="search_input"  />
         <!-- <input type="text" value="" class="search_input" onfocus="this.value=(this.value=='全文搜索&hellip;')? '' : this.value ;" /> -->
        <input type="submit" id="search" value="搜索"alt="搜索" />
      </form>
  </div>
</div>
	</div>

	<div id="Cnav">
		<div class="container">
    <div id="nav">
        <ul>
            <li class="nav_first">
                <a href="">首页</a>
            </li>
            <li class="nav_first">
               <a href="">综合交流平台</a>
            </li>
            <li class="nav_first">
                <a href="">学科交流平台</a>
            </li>
            <li class="nav_first">
                <a href="">资源共享平台</a>
            </li>
        </ul>
    </div>
</div>

	</div>

	<div id="Cmain">
		<div class="container">
				
				<div id="topicCon">
					<div class="topic">
						<div class="reply">
							<span>15240</span>
						</div>
						<div class="substance">
							<div class="substance_top">
								<div class="s_t_title"><a href="">讨论交流主题 arghikashu a wkdfihaiuwh awrhauiwrh arsetgsedwgsdaergasetgwAETSERFErfsgsd的责任感的大概多少如果这些gfhuiawgb rfu</a></div>
								<div class="s_t_author"><img src="" alt="" class="icon_a" title="主题作者：" style="background: url(/hzaucomm/Public/communication/public/icon.png) no-repeat;">
								<span class="s_t_l">发布者</span>
								<div class="s_t_a_hid">发布者信息方式各一个分阿斯乌尔禾ifuse覆盖哈胡一菲桂柳话 UI规划局看都看好陆毅老客户了耦合剂 </div></div>
							</div>
							<div class="clr"></div>
							<div class="substance_down">
								<div class="s_d_reply">最新的回复信息</div>
								<div class="s_d_author"><img src="" alt="" class="icon_r" title="最新回复人：" style="background: url(/hzaucomm/Public/communication/public/icon.png) no-repeat;">
									<span class="s_d_l">回复人</span>
									<div class="s_d_a_hid">回复人信息</div>
									<span  class="s_d_r_time" title="最后回复时间">00:00</span>
								</div>
							</div>
						</div>
					</div>		
				</div>
				<div class="hottpc">
					<div id="h_t_ti">热门话题</div>
					<ul id="h_t_cont">
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num_c">1</span><a class="h_t_con_ex_tit" href="">说为何啵啵猝死家中 </a><span class="h_t_con_ex_heat">999</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num_c">2</span><a class="h_t_con_ex_tit" href="">天宇为何无故。</a><span class="h_t_con_ex_heat">99</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num_c">3</span><a class="h_t_con_ex_tit" href="">biaoti </a><span class="h_t_con_ex_heat">80</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num">4</span><a class="h_t_con_ex_tit" href="">biaoti </a><span class="h_t_con_ex_heat">70</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num">5</span><a class="h_t_con_ex_tit" href="">biaoti </a><span class="h_t_con_ex_heat">60</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num">6</span><a class="h_t_con_ex_tit" href="">biaoti </a><span class="h_t_con_ex_heat">50</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num">7</span><a class="h_t_con_ex_tit" href="">biaoti </a><span class="h_t_con_ex_heat">40</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num">8</span><a class="h_t_con_ex_tit" href="">biaoti </a><span class="h_t_con_ex_heat">30</span></li>
						<li  class="h_t_con_ex"><span class="h_t_con_ex_num">9</span><a class="h_t_con_ex_tit" href="">biaoti </a><span class="h_t_con_ex_heat">25</span></li>

						
					</ul>
				</div>
		</div>
	</div>
	<div class="clr"></div>
	<div id="Cfooter">
		<div id="con">
<div class="container">
  <div class="footbox">
    <h2>快速导航</h2>
    <ul>
      <li><a href="#">&raquo; 首页</a></li>
      <li><a href="#">&raquo; 网站地图</a></li>
      <li><a href="#">&raquo; 互动平台</a></li>
      <li><a href="#">&raquo; 访问说明</a></li>
    </ul>
  </div>
  <div class="footbox">
    <h2>联系方式</h2>
    <address>
    湖北省武汉市洪山区狮子山街一号<br />
    中国·湖北·武汉<br />
    430070<br />
    Tel: 027-87499963<br />
    </address>
  </div>
  <div class="foot-bottom">
    <p><span>技术支持：<a href="http://www.52feidian.com/">华中农业大学 沸点工作室</a></span>
      CopyRight©2017  华中农业大学作物学实验教学中心 <a href="">管理</a>
    </p>
  </div>
</div>
</div>
	</div>
</body>
</html>